...
All first-party cryptographic logic MUST use the `vox-crypto` crate. The invariant: everything in the crypto graph builds on stable Rust on Windows with only the MSVC toolchain — no `cmake`, `nasm`, `perl`, or `zig` chain. First-party code must not `use`/`import` or directly depend on AEGIS, `ring`, `aws-lc-rs`, `openssl`, `md5`, or `sha1`. This is **not** a whole-graph ban: `ring` is deliberately mandated as the rustls backend (it ships pre-generated objects, so consumers need no `nasm`/`perl` — only `cc` with MSVC). Note `aws-lc-rs` is **already in the graph** and builds fine here: `reqwest` 0.13 arrives transitively (tauri, rmcp, chromiumoxide, gix-transport, self_update) and its `rustls` feature expands to `__rustls-aws-lc-rs`, which `workspace-hack` enables. Removing it needs feature or `[patch]` surgery on those five consumers, not a version pin — and the objection to it is build *weight*, not build *failure*. Pure-Rust `chacha20poly1305` is standard for AEAD. Accepted transitive crypto deps (e.g. `aegis`, unavoidably non-optional in `turso_core`) are listed in the SSoT. See:
