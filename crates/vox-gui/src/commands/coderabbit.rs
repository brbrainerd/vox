...
pub async fn coderabbit_plan(
    ...
    run_vox(&app, submit_args(&since, cap, &rank_weights, None, false)).await?;
    let m = std::fs::read_to_string(".coderabbit/semantic-manifest.json")
...
pub async fn coderabbit_run_async(
    ...
    tokio::spawn(async move {
        let res = run_vox(&app, submit_args(&since, cap, &rank_weights, top, true)).await;
...
pub async fn coderabbit_report(app: AppHandle) -> Result<Value, String> {
    let run_state = std::fs::read_to_string(".coderabbit/run-state.json")
        .ok()
