...
// Regression test for Task 19 (free-tier onboarding plan): when no OpenRouter
// key is configured, the LLM & providers banner offers a jump-link to the
// Keys & Secrets section (rather than just plain text), and clicking it
// switches to that section.
test.describe('Vox Settings — LLM banner jump-link', () => {
  test('jumps from the LLM banner to Keys & Secrets when no key is configured', async ({ page }) => {
    await page.addInitScript(() => {
      localStorage.setItem('vox_sidebar_mode', 'default');
      localStorage.setItem('vox_onboarding_dismissed', 'true');
      (window as any).__TAURI_CALLS__ = [];
      (window as any).__TAURI_INTERNALS__ = {
        invoke: async (cmd: string, args?: Record<string, unknown>) => {
          ...
          if (cmd === 'set_gui_preference') return null;
          if (cmd === 'get_orchestrator_status_bin') return new Uint8Array([0x80]);
          if (cmd === 'list_secret_status') return [{ id: 'OPENROUTER_API_KEY', isPresent: false }];
          if (cmd === 'inference_provider_status') return [];
          if (cmd === 'openrouter_key_status') return { configured: false };
          if (cmd === 'get_llm_config') return {};
          if (cmd === 'get_user_config') return [];
          if (cmd === 'get_llm_spend') return null;
          if (cmd === 'get_orchestrator_config') return {};
          return null;
    ...
    const jumpLink = page.getByRole('button', { name: /add one under keys/i });
