...
function renderItemBody(item: StreamItem) {
  return (
    <>
      <div className="font-display text-[13px] font-medium tracking-tight text-zinc-100">{item.title}</div>
      {item.body ? (
        <div className="text-[12px] leading-relaxed text-zinc-400 whitespace-pre-wrap break-words">{item.body}</div>
      ) : null}
    </>
  );
}
        ...
        <div className="flex items-start justify-between gap-3 pl-1">
          <div className="min-w-0 flex-1">
            <div className="mb-1 flex flex-wrap items-center gap-2">
              ...
              <span className="font-display text-[10px] tracking-widest uppercase text-zinc-500">TOKEN</span>
              <span className="font-mono text-[10px] text-zinc-600">{row.items.length} chunks</span>
            </div>
            <button
              type="button"
              className="text-left text-[12px] text-zinc-400 hover:text-zinc-200"
            ...
            {expanded && (
              <div className="mt-2 rounded-lg border border-white/5 bg-black/20 p-2 font-mono text-[11px] text-zinc-300 whitespace-pre-wrap break-words">
                {combinedBody || '(empty)'}
              </div>
            )}
          </div>
          <div className="flex shrink-0 flex-col items-end gap-2">
            <span className="font-mono text-[10px] text-zinc-500">{head.ts}</span>
            {agentId && onOpenAgent ? (
              <Button
                type="button"
                onClick={() => onOpenAgent(agentId)}
                className="rounded-md border border-white/10 px-2 py-1 text-[10px] text-zinc-300 hover:border-brass/30 hover:text-brass"
              >
                View in Flow
              </Button>
            ) : null}
  ...
  return (
    ...
    >
      ...
      <div className="flex items-start justify-between gap-3 pl-1">
          ...
          {row.collapsed ? (
            ...
            >
              {expanded ? renderItemBody(row.item) : (
                <div className="text-[12px] text-zinc-400">{row.item.title}</div>
              )}
            </button>
          ) : (
        ...
        <div className="flex shrink-0 flex-col items-end gap-2">
          <span className="font-mono text-[10px] text-zinc-500">{row.item.ts}</span>
          {agentId && onOpenAgent ? (
            <Button
              type="button"
              onClick={() => onOpenAgent(agentId)}
