...
const DOT: Record<StageStatus, string> = {
  ...
  pending: 'bg-overlay-subtle ring-white/10',
      ...
      {stages.map((stage, i) => (
        <React.Fragment key={stage}>
          <div className="flex items-center gap-1.5">
            ...
            <span className="font-mono text-[10px] text-text-muted">{stage.replace(/_/g, ' ')}</span>
          </div>
          {i < stages.length - 1 && <span className="h-px w-3 bg-overlay-subtle" />}
        </React.Fragment>
      ))}
    </div>
  );
