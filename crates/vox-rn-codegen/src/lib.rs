...
//!
//! This crate is **one of two** lowerings for Vox components. The web lowering
//! (under `vox_codegen::codegen_ts::reactive` + `component`) consumes the same
//! `HirModule` and produces React DOM + Tailwind output; this crate consumes the
//! same `HirModule` and produces React Native + StyleSheet output. The HIR is the
...
//! - `vox-client.ts` — reused unchanged from the existing emit; HTTP layer is identical between web and mobile.
//! - `mobile.ts` — reused unchanged (adapter contract via `target="rn"`).
//! - `vox-app-contract.json`, `openapi.json`, `schemas.ts`, `types.ts` — reused.
//! - `app.json`, `babel.config.js`, `metro.config.js`, `eas.json` — Expo scaffold artifacts (one-shot, not overwritten).
//!
...
use vox_codegen::codegen_ts::emitter::CodegenOptions;
...
pub struct RnCodegenOutput {
    ...
    pub diagnostics: Vec<vox_codegen::web_ir::WebIrDiagnostic>,
}
...
pub fn generate_rn(hir: &HirModule, _options: &CodegenOptions) -> Result<RnCodegenOutput, String> {
    ...
    let endpoint_params: std::collections::HashMap<String, Vec<String>> = hir
        ...
        .collect();
    let screen_root_names = vox_codegen::codegen_ts::screen_root_component_names(hir);
    ...
    if !hir.forms.is_empty() {
        ...
        for f in &hir.forms {
            if let Some(name) = &f.on_submit
                && hir.endpoint_fns.iter().any(|e| &e.name == name)
            {
        ...
        // `<Switch>` is only imported when a form actually renders a boolean
        // field — keeping it out otherwise avoids an unused-import under
        // `noUnusedLocals` in the consuming app's tsconfig.
        let switch_import = if hir.forms.iter().any(form::form_renders_switch) {
            ", Switch"
        } else {
            ""
        };
        let header = format!(
            "// AUTO-GENERATED by Vox @form (RN target).\n\
             ...
             import {{ View, Text, TextInput, Pressable, StyleSheet{switch_import} }} from \"react-native\";\n\
             {client_import}\n"
        );
        files.push((
            "forms.tsx".into(),
    ...
    let bundle = vox_codegen::projection_bundle::project_bundle_from_hir(hir);
    let endpoint_param0: std::collections::HashMap<String, String> = hir
        .endpoint_fns
        .iter()
        .filter_map(|e| e.params.first().map(|p| (e.name.clone(), p.name.clone())))
        .collect();
    if let Some(mobile_content) = vox_codegen::codegen_ts::mobile_emit::emit_mobile_setup_for_target(
        &bundle.shell,
        Some("rn"),
        &endpoint_param0,
    ) {
    ...
    if !hir.endpoint_fns.is_empty() {
        files.push((
            vox_codegen::codegen_ts::vox_client::VOX_CLIENT_FILENAME.to_string(),
            vox_codegen::codegen_ts::vox_client::emit_vox_client_for_target(
                hir,
                vox_codegen::codegen_ts::vox_client::VoxClientTarget::ReactNative,
            ),
        ));
    }

    // Zod schemas — pure types; reuse.
    let zod_schemas = vox_codegen::codegen_ts::zod_emit::generate_zod_schemas(hir);
    ...
    let types_content = vox_codegen::codegen_ts::adt::generate_types(hir);
    ...
    let sm_content = vox_codegen::codegen_ts::state_machine_emit::emit_state_machine_decls(hir);
    ...
    if has_schemas || has_api_fns {
        let openapi =
            vox_codegen::codegen_ts::openapi_emit::generate_openapi(hir, "vox-app", "0.1.0");
