...
use vox_codegen::web_ir::WebIrDiagnostic;
...
fn emit_hir_expr_inline_with_state(
    ...
    let ctx =
        vox_codegen::codegen_ts::hir_emit::EmitCtx::with_endpoints(state_names, endpoint_params);
    vox_codegen::codegen_ts::hir_emit::emit_hir_expr(expr, &ctx)
}
...
fn emit_event_handler_with_state(
    ...
    let ctx = vox_codegen::codegen_ts::hir_emit::EmitCtx::with_async_and_endpoints(
        state_names,
        &async_fn_names,
        endpoint_params,
    );
    let body = vox_codegen::codegen_ts::hir_emit::emit_hir_expr(expr, &ctx);
...
pub use vox_codegen::codegen_ts::screen_inset::root_view_bleeds;
            ...
            diagnostics.push(WebIrDiagnostic {
                ...
                message: format!(
                    "RN lowering does not yet handle tag `<{other}>` — falling back to `<View>`. \
                     Add a mapping to crates/vox-rn-codegen/src/component.rs."
                ),
                span: None,
...
fn emit_lifecycle_hooks(
    ...
    use vox_codegen::codegen_ts::hir_emit::{
...
pub fn emit_rn_component(
    ...
    use vox_codegen::codegen_ts::reactive::collect_component_import_refs;
    ...
    let react_es = vox_codegen::codegen_ts::reactive::emit_react_es_import_lines(es_imports);
    out.push_str(&react_es);
    // Phase 5 SSOT: mandatory-provider guidance for known RN libs (Paper/Tamagui).
    out.push_str(&vox_codegen::codegen_ts::reactive::emit_external_lib_support(es_imports, true));
