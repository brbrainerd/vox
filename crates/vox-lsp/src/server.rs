//! Language Server Protocol runner for Vox.
...
impl LanguageServer for Backend {
    ...
    async fn code_action(&self, params: CodeActionParams) -> Result<Option<CodeActionResponse>> {
        ...
        let actions = crate::quickfixes_for_diagnostics(uri, &params.context.diagnostics);
        if actions.is_empty() {
            Ok(None)
        } else {
            Ok(Some(actions))
    ...
    async fn hover(&self, params: HoverParams) -> Result<Option<Hover>> {
        ...
        let Some(word) = crate::word_at_position(&text, pos.line, pos.character) else {
            return Ok(None);
        };
        let line_str = text.lines().nth(pos.line as usize).unwrap_or("");
        if let Some(md) = crate::builtin_hover_markdown_in_line(line_str, &word) {
    ...
    async fn completion(&self, params: CompletionParams) -> Result<Option<CompletionResponse>> {
        Ok(Some(CompletionResponse::List(
            crate::completions::CompletionEngine::completions(params),
        )))
    }
    ...
    async fn document_symbol(
        ...
        if let Ok(module) = parse(tokens) {
            let symbols = crate::symbols::SymbolEngine::symbols(&module, &text);
    ...
    async fn code_lens(&self, params: CodeLensParams) -> Result<Option<Vec<CodeLens>>> {
        ...
        let tokens = lex(&text);
        if let Ok(module) = parse(tokens) {
            let lenses = crate::code_lens::code_lenses_for_module(&module, &text);
            Ok(Some(lenses))
        } else {
            Ok(Some(vec![]))
        }
    ...
    async fn semantic_tokens_full(
        ...
        let data = crate::grammar::encode_semantic_tokens(&text);
...
impl Backend {
    async fn validate_document(&self, uri: Uri, text: String) {
        ...
        let diagnostics = crate::validate_document_with_hir(&text);
...
/// Start the LSP server loop over stdin/stdout.
pub async fn run() {
    // Initialize logging
