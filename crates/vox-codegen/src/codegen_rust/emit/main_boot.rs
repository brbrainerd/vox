...
pub fn emit_main_boot(module: &HirModule) -> String {
    ...
    if has_http_endpoints {
        ...
        out.push_str(
            "    // The workspace does not ship a `vox-http-runtime::serve(db, hir) -> Handle`\n",
        );
        out.push_str(
            "    // because per-route handlers are code-generated by name (not registered at\n",
        );
        out.push_str(
            "    // runtime) — see docs/src/architecture/http-runtime-extraction-2026.md for\n",
        );
        out.push_str(
            "    // the full constraints + two-phase extraction plan. Production codegen uses\n",
        );
        out.push_str(
            "    // `emit_main` (http.rs) which serves HTTP inline; this `emit_main_boot` path\n",
        );
...
pub fn emit_durable_boot_prelude(
    ...
    if scheduled_fns.is_empty() {
        ...
        for (name, interval) in &scheduled_fns {
            ...
            let register_await = propagation.await_suffix(&format!("scheduled::register({name})"));
            out.push_str(&format!(
...
fn emit_hir_embed_helper(module: &HirModule) -> String {
    ...
    loop {
        ...
        debug_assert!(
            hash_count <= 16,
            "raw-string delimiter escalation exceeded 16 hashes"
        );
        if hash_count > 16 {
            break;
        }
