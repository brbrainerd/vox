...
impl Parser {
    ...
    fn expect(&mut self, expected: &Token) -> Result<Span, ()> {
        if std::mem::discriminant(self.peek()) == std::mem::discriminant(expected) {
            ...
            self.errors.push(ParseError {
                ...
                span: self.span(), expected: vec![expected.to_string()],
    ...
    fn parse_module(&mut self) -> Result<Module, Vec<ParseError>> {
        ...
        if self.errors.is_empty() {
            Ok(Module { declarations: decls, span: start.merge(self.span()) })
        } else {
    ...
    fn recover_to_top_level(&mut self) {
        loop {
            match self.peek() {
                Token::Eof => break,
                Token::Fn | Token::AtComponent | Token::Import | Token::TypeKw
                | Token::Actor | Token::Workflow | Token::Http | Token::AtTest | Token::AtServer | Token::AtV0 => break,
                Token::Dedent => { self.advance(); break; }
    ...
    fn parse_decl(&mut self) -> Result<Decl, ()> {
        ...
        match self.peek().clone() {
            ...
            Token::Fn => { let f = self.parse_fn_decl(false)?; Ok(Decl::Function(f)) }
            Token::Pub => {
                self.advance();
                match self.peek().clone() {
                    Token::Fn => { let f = self.parse_fn_decl(true)?; Ok(Decl::Function(f)) }
                    Token::TypeKw => self.parse_typedef(true),
                    _ => { self.errors.push(ParseError { message: "Expected fn or type after pub".into(), span: self.span(), expected: vec!["fn".into(), "type".into()], found: Some(self.peek().to_string()) }); Err(()) }
                }
            }
            ...
            _ => {
                self.errors.push(ParseError { message: format!("Unexpected token at top level: {}", self.peek()), span: self.span(), expected: vec!["fn".into(), "import".into(), "type".into()], found: Some(self.peek().to_string()) });
                Err(())
            }
        }
    }
    ...
    fn parse_import_path(&mut self) -> Result<ImportPath, ()> {
        ...
        match self.peek().clone() {
            Token::Ident(name) | Token::TypeIdent(name) => { segments.push(name); self.advance(); }
            _ => { self.errors.push(ParseError { message: "Expected identifier in import path".into(), span: self.span(), expected: vec!["identifier".into()], found: Some(self.peek().to_string()) }); return Err(()); }
        }
    ...
    fn parse_mcp_tool(&mut self) -> Result<Decl, ()> {
        ...
        let desc = if let Token::StringLit(s) = self.peek().clone() { self.advance(); s } else { String::new() };
        self.skip_newlines();
        let f = self.parse_fn_decl(false)?;
        Ok(Decl::McpTool(McpToolDecl { description: desc, func: f }))
    }
    ...
    fn parse_fn_decl(&mut self, is_pub: bool) -> Result<FnDecl, ()> {
        ...
        Ok(FnDecl { name, generics, params, return_type, body, is_pub, span: start.merge(self.span()) })
    }

    fn parse_ident_name(&mut self) -> Result<String, ()> {
        match self.peek().clone() {
            Token::Ident(n) | Token::TypeIdent(n) => { self.advance(); Ok(n) }
            _ => { self.errors.push(ParseError { message: "Expected identifier".into(), span: self.span(), expected: vec!["identifier".into()], found: Some(self.peek().to_string()) }); Err(()) }
        }
    }
    ...
    fn parse_params(&mut self) -> Result<Vec<Param>, ()> {
        ...
        loop {
            ...
            params.push(Param { name, type_ann, default, span: start.merge(self.span()) });
            if !self.eat(&Token::Comma) { break; }
        }
        Ok(params)
    }
    ...
    fn parse_type_expr(&mut self) -> Result<TypeExpr, ()> {
        ...
        if self.eat(&Token::LBracket) {
            ...
            Ok(TypeExpr::Generic { name, args, span: start.merge(self.span()) })
        } else {
        ...
        match self.peek().clone() {
            ...
            _ => {
                ...
                if self.eat(&Token::Eq) {
                    ...
                    Ok(Stmt::Assign { target: expr, value, span: start.merge(self.span()) })
                } else {
    ...
    fn parse_pattern(&mut self) -> Result<Pattern, ()> {
        let start = self.span();
        match self.peek().clone() {
            Token::Underscore => { self.advance(); Ok(Pattern::Wildcard { span: start }) }
            Token::LParen => {
                self.advance();
                ...
                self.expect(&Token::RParen)?;
                Ok(Pattern::Tuple { elements: elems, span: start.merge(self.span()) })
            }
            Token::TypeIdent(name) => {
                ...
                if self.eat(&Token::LParen) {
                    ...
                    self.expect(&Token::RParen)?;
                    Ok(Pattern::Constructor { name, fields, span: start.merge(self.span()) })
                } else {
                    Ok(Pattern::Ident { name, span: start.merge(self.span()) })
                }
            }
            Token::Ident(name) => { self.advance(); Ok(Pattern::Ident { name, span: start.merge(self.span()) }) }
            Token::IntLit(v) => { self.advance(); Ok(Pattern::Literal { value: Box::new(Expr::IntLit { value: v, span: start }), span: start }) }
            Token::StringLit(s) => { self.advance(); Ok(Pattern::Literal { value: Box::new(Expr::StringLit { value: s, span: start }), span: start }) }
            _ => { self.errors.push(ParseError { message: "Expected pattern".into(), span: start, expected: vec!["identifier".into(), "_".into()], found: Some(self.peek().to_string()) }); Err(()) }
        }
    }
    ...
    fn parse_expr_bp(&mut self, min_bp: u8) -> Result<Expr, ()> {
        ...
        loop {
            if matches!(self.peek(), Token::With) {
                ...
                lhs = Expr::With { operand: Box::new(lhs), options: Box::new(rhs), span };
                continue;
            }
            ...
            let op = match self.peek() {
                ...
                Token::Lte => BinOp::Lte, Token::Gte => BinOp::Gte,
                Token::And => BinOp::And, Token::Or => BinOp::Or,
                Token::Is => BinOp::Is, Token::Isnt => BinOp::Isnt,
    ...
    fn parse_primary(&mut self) -> Result<Expr, ()> {
        ...
        let mut expr = match self.peek().clone() {
            ...
            Token::StringLit(s) => { self.advance(); Expr::StringLit { value: s, span: start } }
            Token::SingleQuoteStringLit(s) => { self.advance(); Expr::StringLit { value: s, span: start } }
            ...
            Token::Not => {
                ...
                Expr::Unary { op: UnOp::Not, operand: Box::new(operand), span: start.merge(self.span()) }
            }
            Token::Minus => {
                self.advance();
                let operand = self.parse_primary()?;
                Expr::Unary { op: UnOp::Neg, operand: Box::new(operand), span: start.merge(self.span()) }
            }
            Token::LParen => {
                self.advance();
                ...
                if self.eat(&Token::Comma) {
                    ...
                    self.expect(&Token::RParen)?;
                    Expr::TupleLit { elements: elems, span: start.merge(self.span()) }
                } else {
                    self.expect(&Token::RParen)?;
                    e
                }
            ...
            Token::LBracket => {
                ...
                self.expect(&Token::RBracket)?;
                Expr::ListLit { elements: elems, span: start.merge(self.span()) }
            }
            ...
            Token::Ident(name) => { self.advance(); Expr::Ident { name, span: start } }
            Token::TypeIdent(name) => { self.advance(); Expr::Ident { name, span: start } }
            _ => {
                self.errors.push(ParseError { message: format!("Unexpected token in expression: {}", self.peek()), span: start, expected: vec![], found: Some(self.peek().to_string()) });
                return Err(());
            }
        };
    ...
    fn parse_match(&mut self) -> Result<Expr, ()> {
        ...
        loop {
            ...
            arms.push(MatchArm { pattern, guard: None, body: Box::new(body), span: arm_start.merge(self.span()) });
            self.skip_newlines();
        }
        self.eat(&Token::Dedent);
        Ok(Expr::Match { subject: Box::new(subject), arms, span: start.merge(self.span()) })
    }
    ...
    fn parse_for(&mut self) -> Result<Expr, ()> {
        ...
        self.skip_newlines();
        self.eat(&Token::Dedent);
        Ok(Expr::For { binding, iterable: Box::new(iterable), body: Box::new(body), span: start.merge(self.span()) })
    }
    ...
    fn parse_jsx(&mut self) -> Result<Expr, ()> {
            ...
            match self.peek() {
                ...
                _ => {
                    ...
                    let value = if self.eat(&Token::LBrace) {
                        ...
                        self.advance();
                        Expr::StringLit { value: s, span: self.span() }
                    } else {
                        self.parse_expr()?
                    };
                    attrs.push(JsxAttribute { name: attr_name, value });
                }
            }
        }
        if self.eat(&Token::JsxSelfClose) {
            return Ok(Expr::JsxSelfClosing(JsxSelfClosingElement { tag, attributes: attrs, span: start.merge(self.span()) }));
        }
        self.expect(&Token::Gt)?;
        // Children
        ...
        loop {
            ...
            match self.peek() {
                ...
                Token::Lt => { children.push(self.parse_jsx()?); }
                Token::LBrace => { self.advance(); let e = self.parse_expr()?; self.expect(&Token::RBrace)?; children.push(e); }
                Token::For => { children.push(self.parse_for()?); }
                Token::StringLit(s) => { let s = s.clone(); let sp = self.span(); self.advance(); children.push(Expr::StringLit { value: s, span: sp }); }
        ...
        loop {
            ...
            if self.eat(&Token::LParen) {
                loop {
                    ...
                    fields.push(VariantField { name: fname, type_ann: ftype, span: vstart.merge(self.span()) });
                    if !self.eat(&Token::Comma) { break; }
                }
                self.expect(&Token::RParen)?;
            }
            variants.push(Variant { name: vname, fields, span: vstart.merge(self.span()) });
        }
        if had_indent { self.eat(&Token::Dedent); }
        Ok(Decl::TypeDef(TypeDefDecl { name, variants, is_pub, span: start.merge(self.span()) }))
    }
    ...
    fn parse_actor(&mut self) -> Result<Decl, ()> {
        ...
        loop {
            ...
            if self.eat(&Token::On) {
                ...
                self.expect(&Token::Colon)?;
                let body = self.parse_block()?;
                handlers.push(ActorHandler { event_name: event, params, return_type: ret, body, span: hstart.merge(self.span()) });
            } else { break; }
        }
        self.eat(&Token::Dedent);
        Ok(Decl::Actor(ActorDecl { name, handlers, span: start.merge(self.span()) }))
    }
    ...
    fn parse_workflow(&mut self) -> Result<Decl, ()> {
        ...
        self.expect(&Token::Colon)?;
        let body = self.parse_block()?;
        Ok(Decl::Workflow(WorkflowDecl { name, params, return_type: ret, body, span: start.merge(self.span()) }))
    }
    ...
    fn parse_activity(&mut self) -> Result<Decl, ()> {
        ...
        self.expect(&Token::Colon)?;
        let body = self.parse_block()?;
        Ok(Decl::Activity(ActivityDecl { name, params, return_type: ret, body, span: start.merge(self.span()) }))
    }
    ...
    fn parse_http_route(&mut self) -> Result<Decl, ()> {
        ...
        let method = match self.peek() {
            ...
            _ => { self.errors.push(ParseError { message: "Expected HTTP method".into(), span: self.span(), expected: vec!["get".into(), "post".into()], found: Some(self.peek().to_string()) }); return Err(()); }
        };
        let path = match self.peek().clone() {
            Token::StringLit(s) => { self.advance(); s }
            _ => { self.errors.push(ParseError { message: "Expected route path string".into(), span: self.span(), expected: vec!["\"path\"".into()], found: Some(self.peek().to_string()) }); return Err(()); }
        };
        let ret = if self.eat(&Token::To) { Some(self.parse_type_expr()?) } else { None };
        self.expect(&Token::Colon)?;
        let body = self.parse_block()?;
        Ok(Decl::HttpRoute(HttpRouteDecl { method, path, return_type: ret, body, span: start.merge(self.span()) }))
    }
        ...
        loop {
            ...
            match self.peek().clone() {
                Token::Ident(_) => {
                    ...
                    self.expect(&Token::Colon)?;
                    let ftype = self.parse_type_expr()?;
                    fields.push(TableField { name: fname, type_ann: ftype, span: fstart.merge(self.span()) });
                }
                _ => break,
            }
        }
        if had_indent { self.eat(&Token::Dedent); }
        Ok(Decl::Table(TableDecl { name, fields, is_pub, span: start.merge(self.span()) }))
    }
        ...
        let (prompt, image_path) = match self.peek().clone() {
            ...
            Token::Ident(kw) if kw == "from" => {
                ...
                match self.peek().clone() {
                    ...
                    _ => {
                        self.errors.push(ParseError { message: "Expected image path string after 'from'".into(), span: self.span(), expected: vec!["\"path\"".into()], found: Some(self.peek().to_string()) });
                        return Err(());
                    }
                }
            }
            _ => {
                self.errors.push(ParseError { message: "Expected prompt string or 'from' after @v0".into(), span: self.span(), expected: vec!["\"prompt\"".into(), "from".into()], found: Some(self.peek().to_string()) });
                return Err(());
            }
        };
        self.expect(&Token::Fn)?;
        ...
        loop {
            ...
            match self.peek().clone() {
                Token::StringLit(path) => {
                    ...
                    entries.push(RouteEntry { path, component_name, span: entry_start.merge(self.span()) });
                }
                _ => break,
            }
        }
...
mod tests {
    ...
    fn test_parse_match() {
        ...
        if let Decl::Function(f) = &m.declarations[0] {
            if let Stmt::Expr { expr: Expr::Match { arms, .. }, .. } = &f.body[0] {
    ...
    fn test_parse_operator_precedence() {
        let m = parse_str("fn f():\n    ret 1 + 2 * 3");
        if let Decl::Function(f) = &m.declarations[0] {
            if let Stmt::Return { value: Some(Expr::Binary { op: BinOp::Add, right, .. }), .. } = &f.body[0] {
                assert!(matches!(right.as_ref(), Expr::Binary { op: BinOp::Mul, .. }));
            } else { panic!("Expected add(1, mul(2,3))"); }
        }
    }
    ...
    fn test_parse_lambda() {
        ...
        if let Decl::Function(f) = &m.declarations[0] {
            assert_eq!(f.body.len(), 2);
            if let Stmt::Let { value: Expr::Lambda { params, .. }, .. } = &f.body[0] {
                assert_eq!(params.len(), 2);
            } else { panic!("Expected lambda let"); }
        } else { panic!("Expected function"); }
    }
    ...
    fn test_parse_if_else() {
        ...
        if let Decl::Function(f) = &m.declarations[0] {
            if let Stmt::Expr { expr: Expr::If { then_body, else_body, .. }, .. } = &f.body[0] {
    ...
    fn test_parse_mutable_let() {
        ...
        if let Decl::Function(f) = &m.declarations[0] {
            ...
            } else { panic!("Expected mutable let"); }
        }
    }
    ...
    fn test_parse_method_chain() {
        ...
        if let Decl::Function(f) = &m.declarations[0] {
            if let Stmt::Return { value: Some(Expr::MethodCall { method, .. }), .. } = &f.body[0] {
                assert_eq!(method, "filter");
            } else { panic!("Expected method chain"); }
        }
    }
    ...
    fn test_parse_jsx_self_closing() {
        ...
        if let Decl::Component(c) = &m.declarations[0] {
            if let Stmt::Expr { expr: Expr::JsxSelfClosing(_), .. } = &c.func.body[0] {
                // ok
            } else { panic!("Expected self-closing JSX"); }
        }
    }

    #[test]
    fn test_parse_jsx_with_children() {
        let m = parse_str("@component fn A() to Element:\n    <div>\n        <span>hello</span>\n    </div>");
        if let Decl::Component(c) = &m.declarations[0] {
            if let Stmt::Expr { expr: Expr::Jsx(el), .. } = &c.func.body[0] {
                assert_eq!(el.tag, "div");
                assert_eq!(el.children.len(), 1);
            } else { panic!("Expected JSX element"); }
        }
    }
    ...
    fn test_parse_spawn() {
        let m = parse_str("fn f():\n    ret spawn(Worker)");
        if let Decl::Function(f) = &m.declarations[0] {
            if let Stmt::Return { value: Some(Expr::Spawn { .. }), .. } = &f.body[0] {
                // ok
            } else { panic!("Expected spawn"); }
        }
    }
    ...
    fn test_parse_for_loop() {
        let m = parse_str("fn f():\n    for x in items:\n        x");
        if let Decl::Function(f) = &m.declarations[0] {
            if let Stmt::Expr { expr: Expr::For { binding, .. }, .. } = &f.body[0] {
                assert_eq!(binding, "x");
    ...
    fn test_parse_activity() {
        ...
        } else { panic!("Expected activity declaration, got {:?}", m.declarations[0]); }
    }
    ...
    fn test_parse_with_expression() {
        ...
        if let Decl::Function(f) = &m.declarations[0] {
            if let Stmt::Return { value: Some(Expr::With { operand, options, .. }), .. } = &f.body[0] {
                assert!(matches!(operand.as_ref(), Expr::Call { .. }));
                assert!(matches!(options.as_ref(), Expr::ObjectLit { .. }));
            } else { panic!("Expected With expression in return"); }
        } else { panic!("Expected function"); }
    }
    ...
    fn test_parse_table() {
        ...
        } else { panic!("Expected table declaration, got {:?}", m.declarations[0]); }
    }
    ...
    fn test_parse_index() {
        ...
        } else { panic!("Expected index declaration, got {:?}", m.declarations[0]); }
    }
    #[test]
    fn test_parse_v0_prompt() {
        ...
        } else { panic!("Expected V0Component, got {:?}", m.declarations[0]); }
    }
