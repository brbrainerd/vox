...
fn all_variants_serialize() {
    ...
    let variants = vec![
        TranscriptKind::Submitted { intent: "VoxNative".into(), input: "1 + 1".into() },
        TranscriptKind::Output { stream: "Stdout".into(), text: "2\n".into() },
        TranscriptKind::AgentTurn { text: "hello".into() },
    ...
    for v in variants {
        let e = TranscriptEvent { session_id: "s1".into(), seq: 0, kind: v };
        let j = serde_json::to_string(&e).unwrap();
        assert!(!j.is_empty());
    }
}
