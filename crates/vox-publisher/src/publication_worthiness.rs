...
///
/// Delegates to [`crate::scientia_novelty_assess::assess_novelty`] for chrono-filter,
/// conflict detection, and scoring — the same seam used by CLI/GUI/MCP surfaces.
#[must_use]
pub fn apply_prior_art_to_worthiness_inputs(
    ...
    if assessment.excluded_future_hits > 0 {
        out.push(format!(
    ...
    if !assessment.conflicts.is_empty() {
        let uri = assessment
            ...
            .map(|h| h.work_uri.as_str())
            .unwrap_or("");
        inputs.novelty = inputs.novelty.min(NOVELTY_NON_NOVEL_CAP);
        out.push(format!(
            "novelty_gate_contradicted: conflicting_uri={uri}; novelty capped before={before:.4} after={:.4}",
            inputs.novelty
        ));
        return out;
    }

    match assessment.verdict_kind.as_str() {
        "insufficient_evidence" => {
            inputs.novelty = inputs.novelty.min(NOVELTY_NON_NOVEL_CAP);
            out.push(format!(
                "novelty_gate_insufficient_evidence; novelty capped before={before:.4} after={:.4}",
                inputs.novelty
            ));
        }
        "contradicted" => {
            inputs.novelty = inputs.novelty.min(NOVELTY_NON_NOVEL_CAP);
            out.push(format!(
                "novelty_gate_contradicted; novelty capped before={before:.4} after={:.4}",
                inputs.novelty
            ));
        }
        "novel" | "possibly_novel" | "not_novel" => {
            let surviving = bundle
                .normalized_hits
                .len()
                .saturating_sub(assessment.excluded_future_hits);
            if surviving == 0 {
                out.push(format!(
        ...
        other => {
            out.push(format!("novelty_gate_unknown_verdict:{other}"));
        }
    }
    out
}
