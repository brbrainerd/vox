...
use crate::types::{AgentTask, TaskId, TaskPhase, TaskPriority, TaskTurn};
use std::sync::Arc;
...
async fn test_methodological_phase_and_transcript_recovery() {
    ...
    orch.record_workflow_phase_change(tid, TaskPhase::Inspect)
        .await;
    ...
    orch.hydrate_all_tasks_from_journal()
        .await
        .expect("hydrate");
    ...
    // 6. Verify recovery
    {
        ...
        assert_eq!(
            t.current_phase,
            Some(TaskPhase::Inspect),
            "Phase should be recovered"
        );
...
async fn test_phase_transition_persists_across_multiple_steps() {
    ...
    // Should have the LATEST phase (last one recorded)
    {
        ...
        assert_eq!(
            t.current_phase,
            Some(TaskPhase::Act),
            "Should recover the latest phase"
        );
    }
}
