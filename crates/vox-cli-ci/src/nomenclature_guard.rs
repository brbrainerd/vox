...
pub fn run(repo_root: &Path, json: bool) -> Result<()> {
    ...
    Err(anyhow!(
        "nomenclature-guard: {} violation(s) — see errors above. To add a grandfathered exception, add to HISTORICAL_ALLOWLIST in crates/vox-cli-ci/src/nomenclature_guard.rs with policy justification.",
        violations.len()
    ))
}
