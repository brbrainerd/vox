use super::super::hir_emit::{EmitCtx, emit_hir_expr};
use super::super::web_ir::{WebIrDiagnostic, WebIrModule};
...
pub(crate) fn emit_reactive_view_body(
    ...
    let diags = super::super::web_ir::validate::validate_web_ir(web);
    let error_diags: Vec<WebIrDiagnostic> = diags
        .into_iter()
        .filter(|d| !super::super::web_ir::validate::is_advisory_diagnostic(d))
        .collect();
    ...
    let Some(tsx) = super::super::web_ir::emit_tsx::emit_component_view_tsx(web, &rc.name) else {
