...
use super::super::web_ir::WebIrModule;
...
///
/// `web_projection` must be the same Web IR graph as [`super::super::projection_bundle::project_bundle_from_hir`]`(hir).web`
...
pub fn generate_reactive_component(
    ...
    if let Some(view_expr) = &rc.view {
        ...
        let pad_screen = super::super::screen_root_component_names(hir).contains(name)
            && !super::super::screen_inset::root_view_bleeds(view_expr);
        if pad_screen {
            out.push_str(&format!(
                "  return (\n    <div className=\"px-4\">\n{}\n    </div>\n  );\n",
                view_js
    ...
    // A11 part 2: Z-tier surfaces rendered through createPortal need the portal
    // import + the generated layer-root resolver. Inject after the first import
    // line so imports stay grouped at the top.
