...
pub fn generate_openapi(hir: &HirModule, package_name: &str, package_version: &str) -> String {
    ...
    emit_from_contract(&ir, package_name, package_version, !hir.tables.is_empty())
}
...
fn emit_from_contract(
    ...
    spec.insert(
        "paths".into(),
        Value::Object(emit_paths(&ir.endpoints, include_health_paths)),
    );
...
fn emit_paths(endpoints: &[ContractEndpoint], include_health_paths: bool) -> Map<String, Value> {
    ...
    if include_health_paths {
        by_path.insert("/healthz".to_string(), emit_health_path_item("healthz"));
        by_path.insert("/readyz".to_string(), emit_health_path_item("readyz"));
    }
    by_path
        .into_iter()
...
fn emit_health_path_item(op_id: &str) -> Map<String, Value> {
    let mut op = Map::new();
    op.insert("operationId".into(), json!(op_id));
    op.insert("summary".into(), json!(format!("GET /{op_id}")));
    op.insert(
        "responses".into(),
        json!({
            "200": {
                ...
                "content": {
                    "application/json": {
                        "schema": {
                            "type": "object",
                            "required": ["status", "backend", "schema_version", "missing_tables", "baseline_digest"],
                            "properties": {
                                "status": { "type": "string" },
                                "backend": { "type": "string" },
            ...
            "503": json_response_ref("Service degraded/unavailable", "ErrorEnvelope"),
            "default": json_response_ref("Error — see wire-format-v1 §6", "ErrorEnvelope")
        }),
    );
    let mut path_item = Map::new();
    path_item.insert("get".into(), Value::Object(op));
    path_item
}
...
mod tests {
    ...
    fn empty_module_emits_minimal_valid_spec() {
        ...
        let s = emit_from_contract(&ir, "demo", "0.1.0", false);
        let v = parse(&s);
    ...
    fn struct_with_decimal_field_widens_to_string_in_schema() {
        ...
        let v = parse(&emit_from_contract(&ir, "demo", "0.1.0", false));
    ...
    fn output_is_byte_identical_for_identical_input() {
        let ir = ContractIr::default();
        let a = emit_from_contract(&ir, "demo", "0.1.0", false);
        let b = emit_from_contract(&ir, "demo", "0.1.0", false);
        assert_eq!(a, b);
    }

    #[test]
    fn health_paths_emit_when_enabled() {
        let ir = ContractIr::default();
        let v = parse(&emit_from_contract(&ir, "demo", "0.1.0", true));
        assert!(v["paths"]["/healthz"]["get"].is_object());
        assert!(v["paths"]["/readyz"]["get"].is_object());
        assert_eq!(
            v["paths"]["/healthz"]["get"]["operationId"],
            json!("healthz")
        );
        assert_eq!(v["paths"]["/readyz"]["get"]["operationId"], json!("readyz"));
    }
}
