use vox_ast::decl::{Module, Decl, HttpRouteDecl, HttpMethod, ServerFnDecl};
...
pub fn generate_routes(module: &Module) -> String {
    let routes: Vec<&HttpRouteDecl> = module.declarations.iter().filter_map(|d| {
        if let Decl::HttpRoute(r) = d { Some(r) } else { None }
    }).collect();

    let server_fns: Vec<&ServerFnDecl> = module.declarations.iter().filter_map(|d| {
        if let Decl::ServerFn(sf) = d { Some(sf) } else { None }
    }).collect();
    ...
    if !routes.is_empty() {
        ...
        out.push_str("      const response = await fetch(\"https://api.anthropic.com/v1/messages\", {\n");
    ...
    for route in &routes {
        ...
        out.push_str(&format!("app.{method}(\"{path}\", async (req: Request, res: Response) => {{\n"));
        out.push_str("  try {\n");
    ...
    for sf in &server_fns {
        ...
        out.push_str(&format!("app.post(\"{route_path}\", async (req: Request, res: Response) => {{\n"));
        out.push_str("  try {\n");
        // Destructure params from request body
        for param in &sf.func.params {
            out.push_str(&format!("    const {} = req.body.{};\n", param.name, param.name));
        }
        // Emit function body
    ...
    out.push_str(&format!("  console.log(`Vox server running on port ${{PORT}}`);\n"));
    out.push_str("});\n");
...
fn emit_route_stmt(stmt: &vox_ast::stmt::Stmt) -> String {
    match stmt {
        vox_ast::stmt::Stmt::Let { pattern, value, mutable, .. } => {
        ...
        vox_ast::stmt::Stmt::Return { value: Some(expr), .. } => {
            let result = emit_route_expr(expr);
            format!("const result = {result};\n    res.json({{ text: result }});\n")
        }
        vox_ast::stmt::Stmt::Return { value: None, .. } => {
            "res.sendStatus(204);\n".to_string()
        }
        vox_ast::stmt::Stmt::Assign { target, value, .. } => {
            format!("{} = {};\n", emit_route_expr(target), emit_route_expr(value))
        }
...
fn emit_route_expr(expr: &vox_ast::expr::Expr) -> String {
    match expr {
        vox_ast::expr::Expr::MethodCall { object, method, args, .. } => {
