...
mod tests {
    ...
    #[tokio::test]
    async fn undo_on_empty_is_nothing_to_undo() {
        let mut b = CasFallback::new();
        assert!(matches!(b.undo().await, Err(VcsError::NothingToUndo)));
    }

    #[tokio::test]
    async fn multi_path_snapshot_preserves_paths_and_label_in_order() {
        ...
        b.snapshot(Some("multi"), paths.clone()).await.unwrap();
        let changes = b.changes().await.unwrap();
        assert_eq!(changes.len(), 1);
    ...
    #[tokio::test]
    async fn ids_are_monotone_with_gaps_after_undo() {
        ...
        let mut b = CasFallback::new();
        assert_eq!(b.snapshot(None, vec![]).await.unwrap().0, 1);
        assert_eq!(b.snapshot(None, vec![]).await.unwrap().0, 2);
        assert_eq!(b.undo().await.unwrap().0, 2); // pops id 2
        // The next snapshot must NOT reuse id 2 — it advances to 3.
        assert_eq!(b.snapshot(None, vec![]).await.unwrap().0, 3);
        let ids: Vec<u64> = b.changes().await.unwrap().iter().map(|c| c.id.0).collect();
        assert_eq!(
            ids,
            vec![1, 3],
    ...
    #[tokio::test]
    async fn diff_conflicts_resolve_are_empty_in_cas_fallback() {
        ...
        let mut b = CasFallback::new();
        b.snapshot(Some("x"), vec![PathBuf::from("a.rs")])
            .await
            .unwrap();
        b.snapshot(Some("y"), vec![PathBuf::from("a.rs")])
            .await
            .unwrap();
        assert!(
