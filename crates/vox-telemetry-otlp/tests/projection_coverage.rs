...
#[test]
fn model_prompt_no_canary_leak() {
    assert_no_canary_leak!(TelemetryEvent::ModelPrompt(ModelPromptEvent {
        canonical_model_id: "claude_sonnet_4".to_string(),
        profile_variant_id: "confirmed_v1".to_string(),
        task_category: "coding".to_string(),
        quality_bucket: "good".to_string(),
    }));
}

#[test]
fn model_prompt_projects_to_correct_category() {
    let ev = TelemetryEvent::ModelPrompt(ModelPromptEvent {
        canonical_model_id: "gemini_flash_2".to_string(),
        profile_variant_id: "none".to_string(),
        task_category: "research".to_string(),
        quality_bucket: String::new(),
    });
    let (cat, map) = project_event(&ev).expect("model_prompt must project");
    assert_eq!(cat, "model_prompt");
    assert_eq!(map["canonical_model_id"], "gemini_flash_2");
    assert_eq!(map["profile_variant_id"], "none");
}
