...
pub fn emit_main(module: &HirModule, package_name: &str) -> String {
    ...
    if has_tables {
        out.push_str("use rusqlite::Connection;\n");
    ...
    if has_routes {
        ...
        if has_tables {
            out.push_str("    let app = app.layer(Extension(db));\n");
        }
...
pub fn emit_lib(module: &HirModule) -> String {
    ...
    if !module.tables.is_empty() {
        out.push_str("use rusqlite::{Connection, params};\n");
    ...
    out.push_str("    let val = serde_json::to_value(v).unwrap();\n");
...
/// Generate a Rust struct for a @table type.
/// Generate a Rust struct for a @table type w/ methods.
fn emit_table_struct(table: &HirTable) -> String {
    let mut out = String::new();
    out.push_str("#[derive(Debug, Clone, Serialize, Deserialize)]\n");
    out.push_str(&format!("pub struct {} {{\n", table.name));
    ...
    // Construct param slice for execute
    out.push_str("        let params_refs: Vec<&dyn rusqlite::ToSql> = params.iter().map(|p| p.as_ref()).collect();\n");
...
/// Generate the DB initialization code for `main()`.
                    ...
                    match key.as_str() {
                            ...
                            match value {
                                HirExpr::StringLit(s, _) => {
                                    opts_builder.push_str(&format!(
                                        ".with_timeout(vox_runtime::ActivityOptions::parse_duration(\"{}\").unwrap())",
                                        s
                                    ));
                                }
                                _ => {
                        ...
                        "initial_backoff" => {
                            if let HirExpr::StringLit(s, _) = value {
                                opts_builder.push_str(&format!(
                                    ".with_initial_backoff(vox_runtime::ActivityOptions::parse_duration(\"{}\").unwrap())",
                                    s
                                ));
                            }
                        }
                        "max_backoff" => {
                            if let HirExpr::StringLit(s, _) = value {
                                opts_builder.push_str(&format!(
                                    ".with_max_backoff(vox_runtime::ActivityOptions::parse_duration(\"{}\").unwrap())",
                                    s
                                ));
                            }
                        }
