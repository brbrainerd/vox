...
pub fn generate_agentic_synth_file(output_path: &Path) -> anyhow::Result<usize> {
    ...
    for tool_name in TOOL_REGISTRY_SLIM {
        ...
        let rec = tool_record(&format!("Execute the {} tool", tool_name), tool_name, args);
    ...
    for (task, tool, args) in &skill_mgmt {
        writeln!(
            file,
            "{}",
            serde_json::to_string(&tool_record(task, tool, args.clone()))?
        )?;
        count += 1;
    }
...
mod tests {
    ...
    fn registry_slim_drives_mcp_records() {
        assert!(
            !TOOL_REGISTRY_SLIM.is_empty(),
            "TOOL_REGISTRY_SLIM must not be empty"
        );
