...
//!
//! Ported from `vox-populi/src/transport/envelope.rs` (D-7-rescope). The plugin
//! owns this canonical copy; `vox-populi`'s copy is retained for backward compat
//! until Phase 5 P5-T1c removes the JWT-HS256 path and the transport is merged.
...
mod tests {
    ...
    fn sign_and_verify_round_trip() {
        ...
        let env = SignedA2AEnvelope::sign("job_submit", b"hello mesh", &sk, &vk);
        assert_eq!(env.version, 1);
        let payload = env.verify_self_signed().expect("should verify");
    ...
    fn tampered_payload_fails() {
        let (sk, vk) = generate_signing_keypair();
        let mut env = SignedA2AEnvelope::sign("ack", b"original", &sk, &vk);
        env.payload_b64 = base64::engine::general_purpose::STANDARD.encode(b"tampered");
        assert!(matches!(
            env.verify_self_signed(),
            Err(EnvelopeVerifyError::SignatureMismatch)
        ));
    }
    ...
    fn wrong_version_fails() {
        let (sk, vk) = generate_signing_keypair();
        let mut env = SignedA2AEnvelope::sign("ack", b"data", &sk, &vk);
        env.version = 2;
        assert!(matches!(
            env.verify_self_signed(),
            Err(EnvelopeVerifyError::UnsupportedVersion(2))
        ));
    }
}
