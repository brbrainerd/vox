...
pub async fn sweep_unreferenced(db: &VoxDb) -> Result<i64, StoreError> {
    ...
    Ok(row.get::<i64>(0).map_err(|e| StoreError::Db(e.to_string()))?)
}
...
mod tests {
    ...
    async fn shared_object_survives_until_no_references() {
        ...
        assert!(db.get(&h).await.is_ok(), "must survive at refs=1 (w2 still references it)");
    ...
    async fn live_context_window_item_protects_object() {
        ...
        sweep_unreferenced(&db).await.unwrap();
        assert!(db.get(&h).await.is_ok(), "live item must be protected from GC");
    }
    ...
    async fn trimmed_item_does_not_protect_object() {
        ...
        context_window_store::mark_item_trimmed(&db, "itrim", 2).await.unwrap();

        // No archive_membership edges and the only reference is trimmed → eligible for GC.
        sweep_unreferenced(&db).await.unwrap();
        assert!(db.get(&h).await.is_err(), "trimmed item must not protect object from GC");
    }
}
