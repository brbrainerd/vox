    ...
    pub(super) fn check_arguments(&mut self, expected: &[Ty], actual: &[HirArg], span: Span) {
        if expected.len() != actual.len() {
            self.diags.push(
                ...
                .with_code(crate::typeck::diagnostics::codes::TYPES_ARG_COUNT_MISMATCH),
            );
            for arg in actual {
        ...
        for (e, a) in expected.iter().zip(actual.iter()) {
            ...
            if let Err(msg) = self.uf.unify(e, &a_ty) {
                ...
                self.diags.push(Diagnostic {
                    ...
                    code: Some(crate::typeck::diagnostics::codes::TYPES_ARG_TYPE_MISMATCH.into()),
                    fixes: vec![],
                    line_col: None,
                    missing_cases: vec![],
                    ast_node_kind: None,
    ...
    pub(super) fn bind_pattern(&mut self, pattern: &HirPattern, ty: &Ty, mutable: bool) {
        ...
        match pattern {
            ...
            HirPattern::Constructor(name, fields, span) => match &ty {
                ...
                Ty::Result(ok, _err) if name == "Ok" && fields.len() == 1 => {
                    self.bind_pattern(&fields[0], ok.as_ref(), mutable);
                }
                Ty::Result(_ok, err) if (name == "Error" || name == "Err") && fields.len() == 1 => {
                    self.bind_pattern(&fields[0], err.as_ref(), mutable);
                }
