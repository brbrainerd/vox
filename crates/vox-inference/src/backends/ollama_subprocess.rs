...
pub struct OllamaSubprocessBackend;

#[async_trait]
impl InferenceBackend for OllamaSubprocessBackend {
    fn id(&self) -> BackendId {
        BackendId::OllamaSubprocess
    }
    ...
    fn load(&self, bundle: &ModelBundle) -> Result<LoadedModel, InferenceError> {
        Ok(LoadedModel {
            backend: self.id(),
            label: format!("ollama-{}", hex_prefix(&bundle.bundle_hash)),
        })
    }
    ...
    async fn predict(
        ...
        Ok(format!("[ollama stub] {}", prompt.text))
    }
