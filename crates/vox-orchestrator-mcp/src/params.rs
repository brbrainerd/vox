...
//!
//! Tool input schemas are generated by `crate::input_schemas::tool_input_schema` with
...
impl<T: Serialize> ToolResult<T> {
    /// Wraps a successful value in a [`Self`].
    ...
    /// Successful [`Self`] with additional machine-readable metadata.
    ...
    /// Builds a failed [`Self`] with the given message.
    ...
    /// Failed [`Self`] with a short remediation for clients (docs links, env keys, etc.).
    ...
    /// Failed [`Self`] with remediation and structured `meta` (e.g. `diagnostics_snapshot`).
