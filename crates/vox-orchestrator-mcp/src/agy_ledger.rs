...
pub fn digest_from_body(body: &str) -> LedgerDigest {
    ...
    for line in body.lines() {
        ...
        // Match only lines that start with a category field to prevent false positives
        // from task descriptions or plan paths that contain "category" mid-line.
        // "category:" appears in errors_encountered blocks; "categories:" in review addenda.
        let is_category_line = t.starts_with("category:") || t.starts_with("categories:");
        if is_category_line {
