...
pub fn render_entry(id: &str, e: &LedgerEntry) -> String {
    ...
    let verification = e.verification.clone().unwrap_or_else(|| {
        format!(
            "{{ tests: \"n/a\", clippy: \"n/a\", arch_check: \"n/a\", smoke: \"exit {}\" }}",
            e.exit_code
        )
    });
    format!(
...
pub fn render_review(id: &str, r: &ReviewRecord) -> String {
    ...
    format!(
        ...
        id = id,
        date = r.date,
        verdict = yaml_inline(&r.verdict),
        cats = cats,
        findings = yaml_inline(&r.findings),
        lessons = lessons,
    )
}

/// Append a `{id}-review` addendum under the same lock (append-only).
pub async fn append_review_locked(
    repo_root: &Path,
    id: &str,
    r: &ReviewRecord,
) -> std::io::Result<()> {
...
";
    ...
    async fn append_roundtrip_advances_id() {
        ...
        let id = append_entry_locked(
            &dir,
            LedgerEntry::new("s", "t", "green", false, 0, 1, 60, "2026-06-19"),
        )
        .await
        .unwrap();
        assert_eq!(id, "AGH-0008");
        let id2 = append_entry_locked(
            &dir,
            LedgerEntry::new("s", "t", "green", false, 0, 1, 60, "2026-06-19"),
        )
        .await
