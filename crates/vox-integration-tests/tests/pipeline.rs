...
fn codegen_routes_produces_app_tsx() {
    ...
    assert!(
        app.1.contains("react-router-dom"),
        "Should import react-router-dom"
    );
...
fn dashboard_full_pipeline_e2e() {
    ...
    assert!(
        ...
        "Should produce server.ts for @server fn"
    );
    assert!(
        filenames.contains(&"App.tsx"),
    ...
    assert!(
        app.1.contains("BrowserRouter"),
        "App should use BrowserRouter"
    );
    ...
    let server = output.files.iter().find(|(n, _)| n == "server.ts").unwrap();
    assert!(
        server.1.contains("/api/stats"),
        "server.ts should contain /api/stats route"
    );
    assert!(
        server.1.contains("/api/stats"),
        "server.ts should contain /api/stats route"
...
fn chatbot_full_pipeline_e2e() {
    ...
    let src = std::fs::read_to_string(path).unwrap_or_else(|_| {
        ...
        std::fs::read_to_string("examples/chatbot.vox")
