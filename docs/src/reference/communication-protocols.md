...
| Vox internal A2A bus | in-code types only | [`orchestration-unified.md`](orchestration-unified.md) | Keep as the low-latency same-process lane (not LF A2A v1.0) |
| LF A2A Agent Card (planned) | [`contracts/orchestration/a2a-agent-card.v1.json`](../../../contracts/orchestration/a2a-agent-card.v1.json) | this page | External Linux Foundation A2A v1.0 discovery — stub only; do not conflate with the internal bus |
