...
- **RPC-as-Function**: Calling a `query`/`mutation`/`server` fn from a `component` looks like a local function call but is actually a type-safe API call generated into the UI layer via `vox-client.ts`.
