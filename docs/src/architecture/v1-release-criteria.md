...
- **[CR-L1] HumanEval-Vox**: A canonical 164-program benchmark suite (`contracts/eval/humaneval-vox/`) must reach ≥ 80% compile + test-pass rate when prompted to MENS or a reference LLM. Reported quarterly via `vox audit --gate humaneval`. *(164 per the HumanEval-Python anchor; see [`vox-as-llm-target-audit-and-plan-2026.md`](vox-as-llm-target-audit-and-plan-2026.md) §9 G18 for the anchoring decision. The earlier "200-program" figure in this row was a pre-anchoring placeholder.)*
- **[CR-L2] On-Distribution Rate**: ≥ 95% of MENS-emitted Vox programs must clear `vox check --strict` + the 47-rule vox-code-audit + retirement-guard with zero errors and zero high-confidence warnings. Reported quarterly via `vox audit --gate mens-on-distribution`.
