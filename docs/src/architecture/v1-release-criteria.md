...
- **[CR-L1] HumanEval-Vox**: A canonical 164-program benchmark suite (`contracts/eval/humaneval-vox/`) must reach ≥ 80% compile + test-pass rate when prompted to MENS or a reference LLM. 164 matches HumanEval-Python for direct comparability (council ratification 2026-05-15; 200 was rejected as arbitrary). Reported quarterly via `vox audit humaneval`.
