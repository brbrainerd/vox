...
| [`vox-mcp-llm-bridge`](../../../crates/vox-mcp-llm-bridge/) | MCP LLM routing, provider adapters, free-tier policy. Decoupled from vox-orchestrator-mcp via `McpServerContext` trait (2026-06-15). Consumer: vox-orchestrator-mcp |
...
| LLM retry-loop error events | `crates/vox-mcp-llm-bridge/src/infer.rs` — `emit_llm_error_event()` helper + `retry_attempt` counter; emitted at each fallback site and terminal failure. |
...
| Add an LLM provider adapter | `crates/vox-mcp-llm-bridge/src/providers/<name>.rs` |
