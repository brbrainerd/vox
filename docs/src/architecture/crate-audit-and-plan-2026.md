...
**State after session 9 (2026-05-24):** arch-check **clean ✓**. **A-12 complete**: extracted `vox-orchestrator/src/dei_shim/` (~3,500 LoC research pipeline) as new `vox-dei-shim` crate (L3, 5,016 LoC incl. tests). `selection/` WIP code excluded (not in original module tree; references types not yet promoted to vox-orchestrator-types). Consumers (`vox-cli`, `vox-orchestrator-mcp`) updated to import from `vox-dei-shim` instead of `vox_orchestrator::dei_shim`. 8 scientia integration tests migrated to vox-dei-shim/tests/. **D-7-rescope Step 1 complete**: `envelope.rs` + `auth_ed25519.rs` ported to `vox-plugin-populi-mesh`; Step 2 (remove vox-populi dep) deferred — requires migrating NodeRecord to vox-mesh-types. **Remaining genuine work:** D-7-rescope Step 2 (deferred), X-1 (tracking PR), P5 XLs A-19/A-20. All P0–P4 tasks complete.
