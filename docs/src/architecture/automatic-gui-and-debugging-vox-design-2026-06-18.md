...
4. **MCP tools** on the existing server — `vox_gui_components`, `vox_gui_tokens`, and **`vox_validate_vuv`** (runs the registry-driven contrast/occlusion/a11y validators on submitted VUV and returns diagnostics), giving external generators shadcn-MCP-style no-hallucinated-props access **plus** a hard correctness check Vox alone can offer.
