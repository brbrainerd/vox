...
4. If the GHCR package is private, configure registry credentials in Coolify so **`pull_policy: always`** can fetch **`ghcr.io/vox-foundation/vox-eval`**. **Preferred fix: make the package public instead** (GitHub → `vox-foundation` org → **Packages** → `vox-eval` → **Package settings** → **Change visibility** → **Public**) — this image carries no secrets/workspace by design (see intro), so there's no reason to gate the pull, and it's a one-time setting vs. a credential Coolify must keep valid. `docker/login-action` in `docker-eval.yml` pushes via the ephemeral `GITHUB_TOKEN`, which creates GHCR packages **private by default** regardless of repo visibility — this has bitten production deploys with `Error response from daemon: error from registry: unauthorized` / `exit status 1` on the `eval Pulling` step even though the image built and pushed successfully moments earlier. Visibility is a package-level setting (not per-tag), so this is a one-time fix, not something to redo per release.
