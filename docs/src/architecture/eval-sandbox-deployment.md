...
4. `ghcr.io/brbrainerd/vox-eval` must be **public** for Coolify's `pull_policy: always` to fetch it without registry credentials (GitHub → `brbrainerd` account → **Packages** → `vox-eval` → **Package settings** → confirm visibility is **Public**). GHCR packages are **private by default** on push regardless of repo visibility, so this needs a one-time check after the package is first created — this has bitten production deploys with `Error response from daemon: error from registry: unauthorized` / `exit status 1` on the `eval Pulling` step even though the image built and pushed successfully moments earlier. **Why a personal namespace, not `vox-foundation/vox-eval`:** the org's free-tier plan blocks changing container-package visibility entirely, so the org-owned package could not be flipped public; a personal-account package has no such restriction. Visibility is a package-level setting (not per-tag), so this is a one-time fix, not something to redo per release.
