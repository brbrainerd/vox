...
description: "Recovered 2026-06-20 design SSOT for unifying chat/agent/archived context as a ContextWindow object model, plus the T4.2 status note on what has actually landed (lossless compaction archival, message-assembly wiring) versus what remains future scope."
category: "Architecture SSOTs"
status: "roadmap"
training_eligible: true
training_rationale: "Recovered architecture design SSOT plus the T4.2 status note distinguishing landed vs. open scope; useful project context for future context-management work."
---

# Context Window Management — Design Spec (recovered 2026-06-20)

> **Recovery provenance (T4.2).** This document was authored 2026-06-20 and
> committed at `4db4b11322`..`f8b7ae35f9` under
> `docs/superpowers/specs/2026-06-20-context-window-management-design.md`, but
> that path was never carried into `docs/src/architecture/` and no longer
> exists on this branch's working tree. Recovered verbatim via
> `git show f8b7ae35f9:docs/superpowers/specs/2026-06-20-context-window-management-design.md`
> as the design SSOT for
> [`vox-axis-harness-reliability-spec-plan-2026-07-02.md`](vox-axis-harness-reliability-spec-plan-2026-07-02.md)
> task **T4.2 — Context management = recover + wire, not build**. See
> [§0 T4.2 Status Note](#0-t42-status-note-2026-07-03) below for what has
> actually landed versus what remains open scope from this design.

## 0. T4.2 Status Note (2026-07-03)

T4.2's scope was narrow relative to this document's full program: recover
this spec as SSOT, and fix/wire the two audit defects the spec's own §2.1
keystone-gap table (items 2 and 10) and §13 adversarial-audit table (row 5)
call out against `crates/vox-orchestrator/src/compaction.rs`:

- **Landed:** `CompactionEngine::compact()` (`compaction.rs`) now returns the
  actual dropped `Turn`s in `CompactionResult::dropped_turns` (previously only
  a `usize` count that silently discarded the content) — the lossless
  contract described in this doc's §5 "Cold" tier bullet and §13 row 5.
- **Landed:** `Session::compact()` and the new `Session::compact_auto()`
  (`crates/vox-orchestrator/src/session/state.rs`) archive every dropped turn
  into `Session::archived_turns` before replacing the live working set with a
  summary turn, instead of `self.turns.clear()`'s prior hard discard.
- **Landed:** `SessionManager::assemble_llm_messages()`
  (`crates/vox-orchestrator/src/session/manager/mutations.rs`, behind the
  `runtime` feature) is the new message-assembly entry point: it runs
  `compact_auto` automatically when the session is over
  `CompactionEngine`'s threshold, persists a `Compacted` session event, and
  returns the (possibly compacted) message list ready for
  `vox_actor_runtime::llm::llm_chat` / `llm_stream_activity`. This is the
  "wire it into message assembly before `llm_chat`/`llm_stream`" deliverable.
  Covered by an integration test
  (`crates/vox-orchestrator/tests/context_compaction_wiring_test.rs`) that
  drives a session over the limit, asserts a compaction event fires, and
  confirms dropped turns are retrievable afterward via
  `Session::archived_turns()` before completing a real `llm_chat` call.
- **Not in scope for T4.2** (still open, per this doc's §11/§14.2
  sequencing): the `context_windows`/`context_window_items` DB spine, CAS
  refcount + GC, the 3-source `WindowProjector`, `git_sha_at_open/close`
  session→commit linkage, the FTS5/embeddings/graphify `RetrievalRouter`,
  per-window `override_model`, and the graph navigator GUI surface. Archival
  in the landed slice lives on the in-memory `Session` (and its DB-replayed
  `agent_session_events` `Compacted` event, which — per this doc's §13 row
  1 caveat — still only replays a summary string, not raw dropped-turn
  content, since the wire event schema was not widened under T4.2). A durable
  CAS-backed archive is the natural home for a future chunk of this design,
  not a T4.2 requirement (T4.2's acceptance criterion is turns being
  retrievable *within the running process*, which the in-memory
  `archived_turns` field satisfies).
- **GUI:** per this doc's own §8 note and
  [`2026-06-19-dockable-workspace-context-memory-ssot-design.md`](../../superpowers/specs/2026-06-19-dockable-workspace-context-memory-ssot-design.md),
  `ContextWindowMeter.tsx` already exists and the Context-Window Editor is
  separately planned there — no new UI was added or duplicated under T4.2.

The rest of this document is the original 2026-06-20 spec, unmodified below
this note, preserved for the larger unified-context-window program it
describes (spine, tiering, retrieval, graphify join) which remains future
work.
...
> **Historical note (T4.2):** this section describes an execution pathway
> (`vox_agy_pipeline` / Gemini Flash 3.5 handoff) targeted at the time this doc
> was written. T4.2 itself was implemented directly in-session rather than via
> that pathway; the sequencing in §14.2 remains the reference decomposition
> for any future chunk of this design.
