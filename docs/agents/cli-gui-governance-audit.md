...
- The clap CLI tree was never ingested into the Graphify surface graph (`.vox/cache/graphify/vox-gui-surface/graph.json` has `cmd:`/`tool:` nodes from the Rust/TSX walk, not the clap derive enums). This audit is the missing CLI-side enumeration; a follow-up could emit these 549 commands as `cli:` nodes and join them to the existing `tool:`/surface nodes for a unified coverage graph.
