...
**Former known gap — now closed (2026-07-04 follow-up):** `TasksView.tsx` used to run its own independent `Promise.all([invoke('hopper_list'), feedbackList()])` poll, a 4th feed the "collapse the three... into ONE" consolidation had never touched. Closed via the option the original note anticipated: `useAttentionInbox`'s `AttentionInbox` shape gained a `hopperTasks: HopperTaskDto[]` field (the hook already fetched this internally to derive `blockedTasksCount`, it just never exposed the raw list), and `TasksView` now takes an optional `attention?: AttentionInbox` prop mirroring `NeedsYouSurface`'s dual-mode pattern — when provided, it sources rows from `attention.hopperTasks`/`attention.needsYou` and skips its own fetch/poll/listeners entirely; when absent (no other render site currently), the original self-fetch path is preserved unchanged. The DTO→row mapping was extracted into a shared `mapHopperTasksToRows()` helper (`tasksHelpers.ts`) so both modes derive lifecycle/priority identically. `surfaceComponents.tsx`'s `case 'tasks':` now passes `attention={props.attention}`. 3 new tests (2 in `TasksView.test.tsx`, 1 in `useAttentionInbox.test.ts`); all pre-existing tests unchanged and green.
