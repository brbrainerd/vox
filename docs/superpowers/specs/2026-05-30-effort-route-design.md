...
- **Q3.** Where does the Vox-capability flag live on the registry entry? **Tentative:** a `ModelCapabilities.writes_vox: bool` field (default false; true for MENS entries) — verify the registry surface during planning; if adding a capability field is heavy, fall back to a config allowlist of Vox-capable model ids.
