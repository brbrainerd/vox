...
`daemon.rs`'s `ping()` call site (already parsing the JSON response) reads the new `version` field and compares it against the GUI's own compile-time version (`env!("CARGO_PKG_VERSION")`). On mismatch, instead of silently proceeding: surface a visible, dismissible warning via `useOrchestratorStatus.ts`'s existing status plumbing (the same hook that already drives the app's freshness pill). Confirmed during implementation planning: there is no existing unified "offline/reconnecting banner" component to extend — `BackendBanner` is a narrower, unrelated mechanism (it detects "no Tauri backend at all," a bare-browser-preview condition, not "a daemon is connected but running a mismatched version"), and the freshness pill is a small inline indicator, not a banner. The fix is a small, new, dedicated banner component, not a re-use of either existing pattern — naming both versions explicitly (e.g. "GUI v0.6.0 / daemon v0.5.9 — restart the daemon to update") so the message is actionable, not just alarming. The app continues to function normally otherwise (soft warning, per the approved choice) — this is a visibility fix, not a new failure mode.
...
- **Known limitation, disclosed rather than silently accepted**: comparing `env!("CARGO_PKG_VERSION")` between the two binaries is a coarse proxy. Because both crates inherit one workspace version that's bumped rarely (release cadence) while source changes daily, this check will most often report "match" even in the exact scenario this spec's own Context section diagnoses as the real bug — a stale staged daemon from several commits ago, where the workspace version string simply hasn't changed between those commits. The check reliably catches only the narrower case of a partial rebuild that happens to span a version bump. A stronger signal (e.g. a build-time-embedded git commit hash compared the same way) would catch the more realistic same-version-different-code drift, but is intentionally out of scope for this spec — a candidate, not a requirement, for a follow-up if the coarse check proves insufficient in practice.
...
**"Required" is contingent on one specific wiring step, not merely on the job existing and passing**: §4's CI job is shared with the sibling `orchestrator-launch-v1-readiness` spec's CR-U6 promotion — both specs' "required gate" framing only holds if that shared job is actually added to this repo's required-checks aggregator (a real, file-based mechanism in `.github/workflows/ci.yml`, not a hypothetical). If that specific wiring step is skipped, the job can exist and pass indefinitely while satisfying neither spec's "required" claim in practice — call this out explicitly during implementation review, don't treat "job is green" as sufficient evidence on its own.
