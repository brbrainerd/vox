...
>
> **Remaining (deliberate, do next):**
> 1. Implement + verify the sccache IP fix above (TDD: update constant + test; rebuild; redeploy fleet; re-run #385 → all-green).
> 2. **Rebase the stack onto the new `main`** — `main` advanced past `0ee0608` (the stack's base) when auto-GUI landed, so FF is no longer clean. Rebase `stack/01..09` onto `origin/main` (disjoint files except possibly `docs/src/architecture/research-index.md` — resolve if it conflicts), force-push, re-verify top == old #376 tree + `.gitleaks.toml` + preserved doc, then FF-merge.
> 3. Sync local `main`; hand this plan to Antigravity/Gemini per the header (`GEMINI.md` now exists).
