...
- [ ] **Step 0.5 — Run all affected tests (scripts test stays ignored for now)**

  The `install_scripts_cover_release_targets` test in `release_build.rs` is still `#[ignore]`-d at this point — the scripts don't exist yet. It will be un-ignored in Task 4 once both scripts are written.
...
- [ ] **Step 4.4 — Un-ignore the install_scripts_cover_release_targets test**

  Now that both `scripts/install.sh` (Task 3) and `scripts/install.ps1` (this task) exist, the test in `release_build.rs` can finally run. The test verifies each triple from `SUPPORTED_RELEASE_TARGETS` appears as a literal string in the scripts — the comment blocks added to both scripts satisfy this.

  In `crates/vox-cli/src/commands/ci/release_build.rs`, find:
  ...
  ```
  cargo test -p vox-cli install_scripts_cover_release_targets
  ```

  Expected: `ok` — all triples found in both scripts.

  Commit:

  ```
  git add crates/vox-cli/src/commands/ci/release_build.rs
  git commit -m "test(vox-cli): enable install_scripts_cover_release_targets now that scripts exist"
