...
**Why:** Structured Outputs makes schema-violating tokens *impossible* ([docs](https://platform.claude.com/docs/en/build-with-claude/structured-outputs)); invaluable for tool-calling reliability from our own served model and for a first-class Vox generation primitive. **What's involved:** a logits-mask/grammar layer in `vox-inference` generate path (compile a JSON-schema/EBNF to a token-level mask), then surface a `generate(schema:)` effect in the Vox language (effect-governed, `GpuCompute`). Files: `crates/vox-populi/src/inference/src/{generate.rs, grammar.rs(new)}`; language surface via the effect system. **Effort:** L. **Risk:** high (tokenizer-aware grammar masking is fiddly). Start with JSON-only.
