...
- **Crate dependency graph SSOT:** `contracts/ci/crate-graph.v1.json`, shape `{"schema_version":1,"crates":{"<name>":["<dep>",...]}}` — 121 workspace crates, 593 edges, direction crate→its deps. Regenerated by `vox ci affected-crates --regen`; includes **all dep kinds (normal+build+dev, indistinguishably)**, so blast numbers are CI-shaped. **`workspace-hack` is a dep of 71 crates** — deliberate feature-unification, excluded from all cut-candidate analyses.
...
- [ ] **Step 1: Regenerate the dependency graph**
...
Semantics (verified 2026-07-02, `affected_cmd.rs::graph_from_metadata`): the adjacency
keeps ALL dependency kinds — normal, build, and dev — indistinguishably. So blast_s is
**CI-shaped** (`--all-targets` rebuilds), not production-shaped. For every recommended
cut in Phase 3, check the consumer's `Cargo.toml` section: a `[dev-dependencies]`-only
edge is cheaper to cut and must be labeled as CI-only savings.
