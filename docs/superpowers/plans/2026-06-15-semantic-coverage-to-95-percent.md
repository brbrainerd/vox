...
- [ ] **Step 4: Run the crate's tests AND lints.** `cargo test -p C` green, **then** `cargo clippy -p C --all-targets -- -D warnings -A clippy::items_after_test_module -A clippy::collapsible_match -A clippy::collapsible_if -A clippy::should_implement_trait -A clippy::doc_overindented_list_items -A clippy::doc_lazy_continuation` clean and `cargo fmt -p C --check` clean. **This is not optional**: waves 1–47 skipped it and accumulated ~150 clippy errors across ~30 crates that silently broke the main Lints gate for every PR (fixed 2026-07-02, PR #430). Generated tests must land clippy-clean — common offenders: `field_reassign_with_default`, `assertions_on_constants`, unused imports, and edition-2024 `std::env::set_var` needing `#![cfg_attr(test, allow(unsafe_code))]`. The allowlist above MUST stay in lockstep with the `lints` job in `.github/workflows/ci.yml`.
