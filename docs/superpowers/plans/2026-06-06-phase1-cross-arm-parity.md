...
**VERIFIED 2026-06-07 (probe `crates/vox-integration-tests/tests/ts_emit_goldens_probe.rs`):** codegen-ts emits all 10 EXPECT goldens **without error, but defines `main` in 0/10**. Dumping the output for `noop` and `decimal_math` shows `BuildMode::App` emits exactly two files — an empty `vox-app-contract.json` and boilerplate `vox-tanstack-query.tsx` — and **drops `fn main()` + its `print()`/logic entirely**. codegen-ts is a *web-app emitter that discards non-web top-level functions*. **Therefore Task 1.A is NOT "synthesize an entry" — codegen-ts needs a genuinely new "script/console" emit mode** that lowers top-level `fn main` + free functions to runnable TS (analogous to codegen-rust's script lane), THEN the Node harness. This is a real codegen feature, materially larger than a test harness, and is the true long pole of the ts arm. The probe's `codegen_ts_golden_emit_census` test is a committed baseline ratchet (asserts `defines_main == 0/10`) that trips when that emit mode lands.
