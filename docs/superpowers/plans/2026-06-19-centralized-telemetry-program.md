...
**Version pinning (verify-then-pin, June 2026 floors):** In Track A you will run
`cargo add <crate>@<floor> --dry-run` (or check crates.io) and record the EXACT resolved
version in `contracts/telemetry/pinned-versions.md`. Floors: `opentelemetry*` ≥ `0.32.0`,
`clickhouse` = `0.13.3`, `governor` ≥ `0.8`. Bump the workspace root `Cargo.toml` otel pins
from `0.29` → resolved `0.32.x` and pick the matching `tracing-opentelemetry` (cargo will
reject incompatible pairs — let the resolver tell you).
...
- [ ] **Step 1: Inventory in-repo infra.** Read `infra/coolify/README.md`, the `*.compose.yml`
  files, `docs/src/reference/deployment-compose.md`, `docs/src/reference/cli-vox-deploy.md`, and
  the `vox deploy` command source. Record: what Coolify project(s) exist, what services are
  defined, whether a Vox project already exists on FableForge, and how `vox deploy` targets it.
...
- [ ] **Step 4: Pin versions.** Create `contracts/telemetry/pinned-versions.md` with EXACT
  resolved versions (see Shared Conventions). **Commit** both files.
```bash
...
### Task B2: Redaction pass (egress boundary) — TDD

**Files:**
- Create: `crates/vox-telemetry-otlp/src/redact.rs`
- Test: `crates/vox-telemetry-otlp/tests/redaction_guardrail.rs`

- [ ] **Step 1: Write the failing guardrail test.** It loads the taxonomy SSOT and asserts the
  redactor drops any field not in a category's allowlist and rejects free-form values:
```rust
use vox_telemetry_otlp::redact::{redact_event, RedactedRecord};

#[test]
fn redactor_drops_unlisted_fields_and_freeform_values() {
    // A synthetic event carrying a secret in an unlisted field must come out empty of it.
    let raw = serde_json::json!({
        "category": "command_usage",
        "verb": "build",
        "secret_token": "ghp_THIS_MUST_NEVER_LEAVE",
        "duration_bucket": "lt1s"
    });
    let out: RedactedRecord = redact_event(&raw).expect("known category");
    let s = serde_json::to_string(&out).unwrap();
    assert!(!s.contains("ghp_THIS_MUST_NEVER_LEAVE"), "secret field leaked past redaction");
    assert!(s.contains("\"verb\""));
    assert!(s.contains("\"duration_bucket\""));
}
```
- [ ] **Step 2: Run** `cargo test -p vox-telemetry-otlp --features remote redactor_drops` → FAIL (no `redact` impl).
- [ ] **Step 3: Implement `redact.rs`** — load taxonomy once (`include_str!` the SSOT), build a
  per-category allowlist map, and emit only allowlisted, type-checked fields:
```rust
use serde::Serialize;
...
fn allowlist() -> HashMap<String, (String, HashSet<String>)> {
    // category -> (otlp_event_name, set<field_name>)
    let v: serde_json::Value = serde_json::from_str(TAXONOMY).unwrap();
...
pub fn redact_event(raw: &serde_json::Value) -> Option<RedactedRecord> {
    let cat = raw["category"].as_str()?;
    let (event_name, allowed) = allowlist().remove(cat)?;
    ...
    Some(RedactedRecord { event_name, attrs })
}
```
  (Make `redact` `pub mod` for the test; cache `allowlist()` in a `OnceLock` in a follow-up — note it in the ledger.)
- [ ] **Step 4: Run** the test → PASS. Add a second test that an entirely-unknown category returns `None`.
- [ ] **Step 5: Commit.**
```bash
git add crates/vox-telemetry-otlp/src/redact.rs crates/vox-telemetry-otlp/tests/redaction_guardrail.rs
git commit -m "feat(telemetry-otlp): egress redaction allowlist + guardrail tests"
```
...
- [ ] **Step 3: Implement** in `config.rs`: a `ConsentState { Unset, Granted, Denied }` enum;
  `install_id()` reads-or-creates a random UUID persisted next to the user config (reuse the
  existing `user_config_path()` dir; generate via a tiny inline 128-bit RNG from `std`
  time+`getrandom` — **no new dep**; if `getrandom` is already in the tree via workspace-hack,
  use it). `remote_consent()` reads the `[telemetry] remote_consent = "granted|denied"` key;
  `set_remote_consent()` writes it. Persisting consent is the ONLY thing that flips
  `remote_upload` effective-true.
...
- [ ] **Step 3: Implement `OtlpRecorder`** — holds an `opentelemetry_sdk` logger provider
  ...
  remote_consent()!=Granted { return; } let raw = event_to_json(e); if let Some(r) =
  redact_event(&raw) { self.emit_log(r); } } }`. `emit_log` builds an OTLP `LogRecord`
  (`event.name`=r.event_name, attributes=r.attrs) and enqueues via the spool (best-effort, never blocks).
- [ ] **Step 4: Run** → PASS.
- [ ] **Step 5: Commit.**
```bash
...
- [ ] In the binary startup (vox-cli + vox-gui + orchestrator daemon), when `telemetry-remote`
  + consent==Granted, register `OtlpRecorder` alongside the existing local sink via
  `CompositeRecorder` (local + remote). Existing categories (`research_metrics`, `model_calls`,
  `agent_orchestration`, `build`, `errors`) now ALSO upload — gated identically. Add a test that
  `CompositeRecorder` fans out to both and that the remote leg is silent without consent.
...
- **Type consistency:** `TelemetryRecorder`/`set_global_recorder`/`CompositeRecorder` are the
  real facade symbols (verified in `crates/vox-telemetry/src/lib.rs`); `ConsentState`,
  `OtlpRecorder`, `redact_event`, `RedactedRecord`, `gen_ddl` are introduced once and reused
  consistently across B/C/E.
- **Gaps to flag at execution:** exact `tracing-opentelemetry` pin (resolve in A4); whether
  FableForge already hosts a Vox project (resolve in A4); OTel-Collector-vs-axum ingest (A4).
