...
| `vox_orchestrator_start` | Start the AgentFleet runtime programmatically from an active AI agent session. |
...
| `vox_map_agent_session` | Map an agent session ID to an existing orchestrator agent. |
| `vox_poll_events` | Poll recent orchestrator events for all agents. |
| `vox_submit_task` | Submit a new task to the Vox Orchestrator. |
| `vox_heartbeat` | Send an active heartbeat from an AI agent session. |
| `vox_record_cost` | Record a cost event from an agent session token usage. |
